...
void describe('Phase 1 manifest', () => {
  void test('classifies the current Phase 1 source files', () => {
    ...
    assert.ok(manifest.files.some((file) => file.path === 'data/tables.json' && file.role === 'entity'));
  ...
  void test('rejects missing and unclassified required source collections', () => {
    ...
    writeFileSync(join(missingClassDirectoryRoot, 'data', 'actions.json'), '{ "action": [] }');
    writeFileSync(
    ...
    writeFileSync(join(missingClassDirectoryRoot, 'data', 'tables.json'), '{ "table": [] }');
