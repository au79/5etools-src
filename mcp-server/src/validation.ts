...
const ADVENTURE_ENTRY_TYPES = new Set([
  'abilityAttackMod',
  'abilityDc',
  'abilityGeneric',
  'cell',
...
const ADVENTURE_ENTRY_FIELDS = new Set([
  'altText',
  'alias',
  'allowRefDupe',
  'area',
...
export function validateAdventureEntryTree(value: unknown, path: readonly PropertyKey[] = []): void {
  if (typeof value === 'string') return;
  if (Array.isArray(value)) {
    value.forEach((entry, index) => validateAdventureEntryTree(entry, [...path, index]));
    return;
  }
  if (typeof value !== 'object' || value === null)
    throw new ValidationError([
      { collection: 'adventure', file: 'adventure text', message: 'Expected an entry node.', path },
    ]);
  const entry = value as Record<string, unknown>;
  if (typeof entry.type !== 'string' || !ADVENTURE_ENTRY_TYPES.has(entry.type))
    throw new ValidationError([
      { collection: 'adventure', file: 'adventure text', message: 'Unsupported entry type.', path: [...path, 'type'] },
    ]);
  const unknown = Object.keys(entry).filter((field) => !ADVENTURE_ENTRY_FIELDS.has(field));
  if (unknown.length > 0)
    throw new ValidationError(
      unknown.map((field) => ({
        collection: 'adventure' as const,
        file: 'adventure text',
        message: 'Unsupported entry field.',
        path: [...path, field],
      })),
    );
  for (const field of ['entries', 'items', 'entry'] as const)
    if (entry[field] !== undefined) validateAdventureEntryTree(entry[field], [...path, field]);
}
